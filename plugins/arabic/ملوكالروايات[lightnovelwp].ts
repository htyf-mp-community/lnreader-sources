...
const plugin = new LightNovelWPPlugin({
  'id': 'kolnovel',
  'sourceSite': 'https://kolnovel.org/',
  'sourceName': 'ملوك الروايات',
  'options': {
    'lang': 'Arabic',
    'reverseChapters': true,
    'customJs':
      "let ignore = $('article > style').text().trim().split(',');ignore.push(...(ignore.pop()?.match(/^\\.\\w+/) || []));ignore.map(tag => $(`p${tag}`).remove());",
  },
  'filters': {
    'genre[]': {
      'type': 'Checkbox',
